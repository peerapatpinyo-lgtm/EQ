...
from plotly.subplots import make_subplots          # BUG FIX: was missing
...
# ==========================================
# 2. ฐานข้อมูล (Data) — EXPANDED DATABASE
#    ครอบคลุมจังหวัดที่มีความเสี่ยงแผ่นดินไหวทั่วประเทศ
#    ค่า Ss, S1 อ้างอิงจากแผนที่เสี่ยงภัยตาม มยผ. 1302-61
# ==========================================
...
with tab3:
    ...
    # BUG FIX: guard against T0=0 (when SDS=0) to prevent division-by-zero in piecewise lambda
    if SDS > 0 and T0 > 0:
        ...
        # Flat zero spectrum for Ss=0 location
        T_values = np.linspace(0.0, 4.0, 500)
...
with tab4:
    ...
    Cs_min = 0.01
    if S1 >= 0.6:
        Cs_min = max(Cs_min, (0.5 * S1) / (R_sys / importance_factor))
    ...
    with sub_tab1:
        ...
        if clean_df.empty:
            ...
            # BUG FIX: make_subplots now properly imported at top of file
