...
if tab4_ready:
  with tab4:
    ...
    with st.container(border=True):
        st.subheader("🧾 รายการคำนวณวิธีแรงสถิตเทียบเท่าอย่างละเอียด (Detailed Engineering Calculation Report)")
        
        # 1. เจาะลึกที่มาของ Cs,gov อย่างละเอียดที่สุด
        st.markdown("#### 1) การวิเคราะห์พารามิเตอร์และการคำนวณสัมประสิทธิ์แรงเฉือนที่ฐาน ($C_{s,gov}$)")
        
        st.markdown("##### 1.1) ตัวแปรตั้งต้นและที่มาของตัวเลข (Input Parameters & Origins):")
        ce_p1, ce_p2 = st.columns(2)
        with ce_p1:
            st.markdown(f"""
            * **$S_{{DS}}$ (ความเร่งสเปกตรัมออกแบบที่คาบสั้น):** $\\mathbf{{{SDS:.4f}}}$ g 
              *(คำนวณมาจากพิกัดอำเภอที่เลือก ผ่านการ Interpolate ขยายความแรงร่วมกับประเภทชั้นดินในระบบฐานข้อมูล)*
            * **$S_{{D1}}$ (ความเร่งสเปกตรัมออกแบบที่คาบ 1 วินาที):** $\\mathbf{{{SD1:.4f}}}$ g
              *(คำนวณจากสเปกตรัมตอบสนองเพื่อใช้กำหนดเพดานสูงสุดของอาคารคาบยาว)*
            * **$I_e$ (ตัวประกอบความสำคัญของอาคาร):** $\\mathbf{{{importance_factor:.2f}}}$ 
              *(พิจารณาตามประเภทการใช้งานอาคารที่เลือกในส่วนจัดหมวดหมู่ความสำคัญ)*
            """)
        with ce_p2:
            st.markdown(f"""
            * **$R$ (ตัวคูณลดผลตอบสนอง):** $\\mathbf{{{R_sys:.1f}}}$ 
              *(ค่าความเหนียวจำเพาะของระบบต้านทานแรงด้านข้างที่คุณเลือกในขั้นที่ 1)*
            * **$R/I_e$ (อัตราส่วนลดทอนแรงสุทธิ):** $\\mathbf{{{cs['RIe']:.3f}}}$
            * **$T_{{design}}$ (คาบเวลาที่ใช้ในการออกแบบ):** $\\mathbf{{{T_design:.4f}}}$ วินาที 
              *(คาบเวลาธรรมชาติของอาคารหลังคูณค่าปรับแก้เพดานสูงสุด $C_u \\times T_a$)*
            """)

        st.markdown("##### 1.2) การแตกสูตรคำนวณเปรียบเทียบตามมาตรฐาน มยผ.:")
        
        if is_soft_clay:
            st.markdown(f"""
            เนื่องจากพื้นที่ก่อสร้างอยู่ในเขต **ดินเหนียวอ่อน (มยผ. 1302)** ค่าสัมประสิทธิ์จะคำนวณผ่านพิกัดกราฟสเปกตรัมดินอ่อน $S_a(T_{{design}})$ โดยตรง:
            
            1. **สัมประสิทธิ์แรงเฉือนพื้นฐาน ($C_{{s,basic}}$):**
               $$ C_{{s,basic}} = \\frac{{S_a(T_{{design}})}}{{R/I_e}} = \\frac{{{cs['Sa_T']:.4f}}}{{{cs['RIe']:.3f}}} = \\mathbf{{{cs['cs_basic']:.5f}}} $$
            
            2. **สัมประสิทธิ์แรงเฉือนขั้นต่ำสุดที่ยอมให้ ($C_{{s,min}}$):**
               $$ C_{{s,min}} = \\max(0.01, \\ 0.044 \\times S_{{DS,bkk}} \\times I_e) = \\max(0.01, \\ 0.044 \\times {cs['SDS_bkk']:.2f} \\times {importance_factor:.2f}) = \\mathbf{{{cs['cs_min']:.5f}}} $$
            """)
        else:
            st.markdown(f"""
            คำนวณตามมาตรฐาน **มยผ. 1301** โดยทำการตรวจสอบพิกัดบน พิกัดล่าง และเพดานสูงสุด:
            
            1. **สัมประสิทธิ์แรงเฉือนพื้นฐาน ($C_{{s,basic}}$):**
               $$ C_{{s,basic}} = \\frac{{S_{{DS}}}}{{R/I_e}} = \\frac{{{SDS:.4f}}}{{{cs['RIe']:.3f}}} = \\mathbf{{{cs['cs_basic']:.5f}}} $$
            
            2. **เพดานค่าสัมประสิทธิ์สูงสุดเพื่อไม่ให้ออกแบบเผื่อเกินพอดี ($C_{{s,max}}$):**
               $$ C_{{s,max}} = \\frac{{S_{{D1}}}}{{T_{{design}} \\times (R/I_e)}} = \\frac{{{SD1:.4f}}}{{{T_design:.4f} \\times {cs['RIe']:.3f}}} = \\mathbf{{{cs['cs_max']:.5f}}} $$
            
            3. **สัมประสิทธิ์แรงเฉือนขั้นต่ำสุดตามกฎหมายเพื่อความปลอดภัย ($C_{{s,min}}$):**
               $$ C_{{s,min}} = \\max(0.01, \\ 0.044 \\times S_{{DS}} \\times I_e) = \\max(0.01, \\ 0.044 \\times {SDS:.4f} \\times {importance_factor:.2f}) = \\mathbf{{{cs['cs_min']:.5f}}} $$
            """)
            if S1 >= 0.6:
                st.markdown(f"""
                4. **ค่าขั้นต่ำเพิ่มเติมกรณีโครงสร้างอยู่ใกล้รอยเลื่อนรุนแรง ($S_1 \\ge 0.6\\,g$):**
                   $$ C_{{s,min,S1}} = \\frac{{0.5 \\times S_1}}{{R/I_e}} = \\frac{{0.5 \\times {S1:.3f}}}{{{cs['RIe']:.3f}}} = \\mathbf{{{cs['cs_min_s1']:.5f}}} $$
                """)

        st.markdown(f"""
        ##### 1.3) สรุปกระบวนการคัดเลือกค่าควบคุม ($C_{{s,gov}}$):
        ตามข้อกำหนดของมาตรฐาน ค่า $C_{{s,basic}}$ จะต้องไม่เกิน $C_{{s,max}}$ และห้ามต่ำกว่า $C_{{s,min}}$ 
        ซึ่งระบบ Logic ใน Engine ทำการคัดเลือกผลลัพธ์ที่ถูกต้องตามกฎหมายออกมาได้คือ:
        $$ C_{{s,gov}} = \\mathbf{{{cs['cs_gov']:.5f}}} \\quad \\rightarrow \\text{{(ตัวควบคุมคือ: {cs['controls']})}} $$
        
        เมื่อนำมาคูณร่วมกับตัวคูณความน่าเชื่อถือระบบโครงสร้าง (Redundancy Factor, $\\rho = {rho:.2f}$) สัมประสิทธิ์ที่ใช้ในการคำนวณแรงจริงจะเท่ากับ:
        $$ C_{{s,design}} = \\rho \\times C_{{s,gov}} = {rho:.2f} \\times {cs['cs_gov']:.5f} = \\mathbf{{{rho * cs['cs_gov']:.5f}}} $$
        """)
        
        st.divider()

        # 2. แรงเฉือนที่ฐาน (V)
        st.markdown("#### 2) การคำนวณแรงเฉือนที่ฐานรวมของอาคาร (Total Base Shear, $V$)")
        st.markdown(f"""
        น้ำหนักประสิทธิผลรวมของอาคาร ($W$) เกิดจากการรวมน้ำหนักบรรทุกคงที่และน้ำหนักบรรทุกจรตามเกณฑ์ควบคุมทุกชั้นเข้าด้วยกัน:
        $$ W = \\sum w_x = \\mathbf{{{total_W:,.2f}}}\\ \\text{{ตัน}} $$
        
        สมการคำนวณแรงเฉือนที่ฐานโครงสร้าง:
        $$ V = C_{{s,gov}} \\times W $$
        แทนค่าตัวเลขจริงลงในสมการ:
        $$ V = {cs['cs_gov']:.5f} \\times {total_W:,.2f} = \\mathbf{{{total_V:,.3f}}}\\ \\text{{ตัน}} $$
        """)
        
        st.divider()

        # 3. ตัวคูณเลขชี้กำลัง (k)
        st.markdown("#### 3) การคำนวณหาค่าเลขชี้กำลังตามแนวดิ่ง (Vertical Distribution Exponent, $k$)")
        if Ta <= 0.5:
        ...
        # 4. สัมประสิทธิ์การกระจายแรง (Cvx) และแรงประจำชั้น (Fx)
        st.markdown("#### 4) หลักการกระจายแรงเข้าสู่แต่ละระดับชั้น ($C_{vx}$ และ $F_x$)")
        st.markdown(f"""
