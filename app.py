...
# 2. ฐานข้อมูล (Data)
# ==========================================
# (จำลอง CSV - นำไปเปลี่ยนเป็น pd.read_csv('seismic_data.csv') ในการใช้งานจริง)
...
def evaluate_sdc(SDS: float, SD1: float, Ie: float) -> str:
    """ฟังก์ชันประเมินประเภทการออกแบบต้านทานแผ่นดินไหว (SDC) ตาม มยผ. 1302"""
    # กลุ่ม Ie <= 1.25 (อาคารทั่วไป/ชุมนุมคน), กลุ่ม Ie = 1.5 (อาคารจำเป็น/ฉุกเฉิน)
    is_essential = (Ie >= 1.5)
...
with st.sidebar:
    ...
    importance_factor = st.selectbox("ตัวคูณความสำคัญ (Ie)", [1.0, 1.25, 1.5], index=0, help="1.0=ทั่วไป, 1.25=ชุมนุมคน, 1.5=โรงพยาบาล/ฉุกเฉิน")
...
if selected_province == "กรุงเทพมหานคร":
    st.warning("⚠️ สำหรับพื้นที่ดินเหนียวอ่อนกรุงเทพฯ ต้องใช้ Response Spectrum เฉพาะตาม มยผ. 1302 ไม่สามารถหา Fa, Fv ปกติได้")
    st.stop()
if site_class == 'F':
    st.error("🛑 ชั้นดิน F ต้องเจาะสำรวจประเมินเฉพาะพื้นที่ (Site-Specific) เท่านั้น")
    st.stop()
...
if sdc == 'ก':
    ...
    st.info("💡 **ข้อกำหนด มยผ.:** อาคารประเภท 'ก' ไม่ต้องวิเคราะห์แรงแผ่นดินไหวแบบเต็มรูปแบบ ให้ทำการออกแบบความสมบูรณ์ของโครงสร้าง (Structural Integrity) โดยคิดแรงกระทำด้านข้างอย่างน้อย **1% ของน้ำหนักอาคาร ($0.01W$)** ก็เพียงพอ")
    
    # คำนวณแบบ 1% ให้ผู้ใช้ดูเลย
    v_min_sdc_a = 0.01 * building_weight
    st.metric("แรงเฉือนที่ฐานขั้นต่ำ (1% W)", f"{v_min_sdc_a:,.2f} ตัน")
    st.stop() # หยุดการทำงาน ไม่ต้องแสดงผลสมการที่ซับซ้อนต่อ
else:
