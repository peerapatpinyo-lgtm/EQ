...
from plotly.subplots import make_subplots  # FIX: was missing, caused NameError in Tab 4
...
def compute_spectrum_sa(T: float, SDS: float, SD1: float, T0: float, TS: float) -> float:
    """FIX: safe scalar version avoids division-by-zero when T0 == 0."""
    if T0 > 0 and T < T0:
...
# Sidebar-level preliminary Cs (for display in Tab 1; Tab 4 uses its own system-specific R)
Cs_calculated = SDS / (r_factor / importance_factor) if r_factor > 0 else 0.0
Cs_max_prelim = SD1 / (Ta * (r_factor / importance_factor)) if (Ta > 0 and r_factor > 0) else Cs_calculated
Cs_min_prelim = 0.01
if S1 >= 0.6:
    Cs_min_prelim = max(Cs_min_prelim, (0.5 * S1) / (r_factor / importance_factor))
Cs_design = min(max(Cs_calculated, Cs_min_prelim), Cs_max_prelim)
...
with tab2:
    ...
    with st.expander("🔍 ดูที่มาและข้อบังคับแยกตามพารามิเตอร์อย่างละเอียด", expanded=False):
        ...
        with col_sdc1:
            ...
            st.info(
                f"🎯 **ตกเกณฑ์: {sdc_actions[sdc_sds]['title']}**\n\n"
        ...
        with col_sdc2:
            ...
            st.info(
                f"🎯 **ตกเกณฑ์: {sdc_actions[sdc_sd1]['title']}**\n\n"
        ...
        st.markdown(
            f"ตามมาตรฐานกำหนดให้เลือกประเภทที่ **เข้มงวดที่สุด (Max)** "
            f"ระหว่างขา คาบสั้น (**{sdc_sds}**) และ คาบยาว (**{sdc_sd1}**)"
        )
...
with tab3:
    ...
    # FIX: use safe scalar function to avoid ZeroDivisionError when T0 == 0
    T_values = np.linspace(0.0, max(4.0, Ta * 1.5, TS * 2), 500)
    Sa_values = np.array([compute_spectrum_sa(t, SDS, SD1, T0, TS) for t in T_values])
    Sa_Ta = compute_spectrum_sa(Ta, SDS, SD1, T0, TS)
...
with tab4:
    ...
    selected_system = st.selectbox(
        "🔷 เลือกระบบโครงสร้างต้านทานแรงด้านข้าง (Seismic Resisting System):",
        list(structural_systems.keys())
    )
    ...
    # FIX: apply full Cs rules using the system-selected R (not the sidebar r_factor)
    ...
    sub_tab1, sub_tab2 = st.tabs([
        "🏗️ 1. รายงานการคำนวณแรงประจำชั้น",
        "📏 2. รายงานการตรวจสอบการโยกตัว"
    ])
    ...
    with sub_tab1:
        ...
        if clean_df.empty:
            ...
            Fx  = cvx * total_V
            # FIX: Vx = cumulative shear from top downward (floors sorted top→bottom)
            Vx  = np.cumsum(Fx)
            ...
            with st.expander("📄 เปิดดูบันทึกข้อความสรุปสูตรคำนวณ (Calculation Note Summary)", expanded=False):
                ...
                st.latex(
                    r"V_{\text{base}} = C_s \times W = "
                    + f"{Cs_gov:.4f} \\times {total_W:,.2f} = {total_V:,.2f} \\text{{ ตัน}}"
                )
                st.latex(
                    r"k_{\text{exponent}} = "
                    + f"{k_exp:.3f} \\quad (\\text{{อ้างอิงจากคาบธรรมชาติอาคาร }} T_a = {Ta:.3f} \\text{{ วินาที}})"
                )
                st.markdown("---")
                for idx, name in enumerate(floor_names):
                    st.markdown(f"**📍 การกระจายแรงสู่ระดับ {name}:**")
                    st.latex(
                        f"C_{{vx}} = \\frac{{{wx[idx]:,.1f} \\times {hx[idx]:.2f}^{{{k_exp:.2f}}}}}"
                        f"{{{sum_w_hx_k:,.1f}}} = {cvx[idx]:.4f}"
                    )
                    st.latex(
                        f"F_x = {cvx[idx]:.4f} \\times {total_V:,.2f} = {Fx[idx]:,.2f} \\text{{ ตัน}}"
                    )
    ...
    with sub_tab2:
        ...
        with st.container(border=True):
            ...
            with col_pic2:
                ...
                fig_model.add_annotation(x=x_orig[1]+dx[3]/2, y=9.7, text="<b>δe</b>", showarrow=False,
                                         font=dict(color='#9333ea'))
                fig_model.add_annotation(x=x_orig[1]+dx[2], y=7.5, ax=x_orig[1]+dx[3], ay=7.5,
                                         xref='x', yref='y', axref='x', ayref='y',
                                         showarrow=True, arrowhead=2, arrowcolor='#db2777')
                fig_model.add_annotation(x=x_orig[1]+(dx[2]+dx[3])/2+0.5, y=7.5,
                                         text="<b>Δ (Drift)</b>", showarrow=False, font=dict(color='#db2777'))
        ...
        if clean_df.empty:
            ...
            edited_drift = st.data_editor(
                ...
                column_config={
                    ...
                    "ระยะโยกพืดหยุ่นจากโปรแกรม δe (ซม.)":
                        st.column_config.NumberColumn(min_value=0.0, format="%.3f")
                },
            ...
            for i in range(n):
                # FIX: story net height = difference between this floor and the one below
                # Floors are stored top→bottom, so floor below is at index i+1
                if i < n - 1:
                    h_net = hx[i] - hx[i + 1]          # inter-story height
                    delta_diff = delta_x[i] - delta_x[i + 1]
                else:
                    h_net = hx[i]                       # ground floor: height from ground to first level
                    delta_diff = delta_x[i]

                h_net = max(h_net, 0.001)               # guard against bad data
                story_h[i]     = h_net
                # delta_x is in cm; h_net is in m → convert h_net to cm
